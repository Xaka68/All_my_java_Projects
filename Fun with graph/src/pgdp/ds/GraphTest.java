...
public abstract class GraphTest<GraphType extends Graph> {
	...
	final void nodeShouldBeNeighborIfConnectionWithEdge() {
		final var graph = createGraph(7);
		final var expectedNeighbors = new int[] { 0, 2, 3, 4, 5, 6 };
		graph.addEdge(1, expectedNeighbors[0]);
		graph.addEdge(1, expectedNeighbors[1]);
		graph.addEdge(1, expectedNeighbors[2]);
		graph.addEdge(1, expectedNeighbors[3]);
		graph.addEdge(1, expectedNeighbors[4]);
		graph.addEdge(1, expectedNeighbors[5]);
