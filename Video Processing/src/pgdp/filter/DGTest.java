 ...
 * HINWEISE:
 * <p>
 * - DAS VIDEO NOOT.MP4 MUSS KORREKT HERUNTERGELADEN UND IM KORREKTEN ORDNER
 * SEIN (wie für das Beispiel in der Main.java)
 * </p>
 * <p>
 * - einige tests erstellen output png und jgp dateien (nach durchlauf der test
 * können diese natürlich auch gelöscht werden) (abschalten mit
 * {@link #outputFiles}, bei test02 darf dies nicht auskommentiert werden!)
 * </p>
 * <p>
 * - die lücken in der testnummerierung dienen nur dazu, nachträglich noch tests
 * einfügen zu können
 */
public class DGTest {
	...
	public void test02() throws IOException, IllegalVideoFormatException {
		...
		// Ausschreiben
		try {
			FrameConsumer fc = new FrameConsumer(in.getProvider(), "test02Written.mp4", in.getProvider().getWidth(),
					in.getProvider().getHeight());
			in.write(fc);
	...
	public void test03() {
		...
		int[] colorsE = new int[] { Color.BLACK.getRGB(), Color.WHITE.getRGB(), Color.WHITE.getRGB(),
				Color.BLACK.getRGB(), Color.BLACK.getRGB(), Color.WHITE.getRGB(), Color.BLACK.getRGB(),
				Color.WHITE.getRGB() };
		for (int i = 0; i < 8; i++) {
		...
		for (int i = 0; i < amount; i++) {
			...
			if (i < 3) {
				...
				for (int j = 8; j < output[i].getWidth(); j++) {
					Assertions.assertEquals(Color.YELLOW.getRGB(),
							output[i].getPixels().getRGB(j, output[i].getHeight() - 1));
				}
				...
				for (int j = 0; j < output[i].getWidth(); j++) {
					for (int k = 0; k < output[i].getHeight(); k++) {
						Assertions.assertEquals(Color.YELLOW.getRGB(), output[i].getPixels().getRGB(j, k),
								"failed at frame " + i + " at index " + j + "," + k);
					}
				}
			}
		}
		...
		for (int i = 0; i < amount; i++) {
			...
			if (i < 2) {
				...
				for (int j = 0; j < 8; j++) {
					Assertions.assertEquals(colorsE[j], output[i].getPixels().getRGB(j, output[i].getHeight() - 1));
					if (i < 1) {
						Assertions.assertEquals(colorsE[j],
								output[i].getPixels().getRGB(8 + j, output[i].getHeight() - 1));
					}
				}
				// rest
				for (int j = 8; j < output[i].getWidth(); j++) {
	...
	public void test51() throws IOException {
		...
		for (int i = 0; i < newWidth; i++) {
			for (int j = endOben; j < endMitte; j++) {
				if (i >= 2 && i <= 3) {
					Assertions.assertEquals(imageCopy.getRGB(i - 2, j - 2), output.getPixels().getRGB(i, j),
							"failed at " + i + "," + j);
				} else {
					Assertions.assertEquals(Color.BLACK.getRGB(), output.getPixels().getRGB(i, j));
				}
		...
		for (int i = 0; i < length; i++) {
			for (int j = 0; j < fp.getWidth(); j++) {
				for (int k = 0; k < fp.getHeight(); k++) {
					Assertions.assertEquals(((Frame) array[i]).getPixels().getRGB(j, k),
							((Frame) output[i]).getPixels().getRGB(j, k));
				}
			}
		}
	}
