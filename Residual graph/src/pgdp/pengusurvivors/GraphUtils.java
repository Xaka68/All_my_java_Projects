...
public class GraphUtils {
	...
	public static void printGraph(FlowGraph graph) {
		...
		for (Vertex vertex : graph.getVertices()) {
			...
			for (Vertex neighbor : neighbors) {
				...
				System.out.println(getName(vertex) + " -> " + getName(neighbor) + " [label=\"" + edge.getFlow() + "/"
						+ edge.getCapacity() + "\"];");
			}
		}
		System.out.println("}");
	...
	/*
	 * Moves all normal edges to the residual network. There will be no edges in the
	 * normal network anymore.
	 */
	...
	/*
	 * Compares the residual network of the source graph with the normal network of
	 * the compareTo Graph.
	 */
		...
		for (Vertex vertex : source.getVertices()) {
			...
			for (Vertex neighbor : neighbors) {
				...
				if (edge.getFlow() != compareToEdge.getFlow()) {
					throw new AssertionError("Flow of edge " + GraphUtils.getName(vertex) + " -> "
							+ GraphUtils.getName(neighbor) + " is not equal. Source: " + edge.getFlow() + " compareTo: "
							+ compareToEdge.getFlow());
				}
				if (edge.getCapacity() != compareToEdge.getCapacity()) {
					throw new AssertionError("Capacity of edge " + GraphUtils.getName(vertex) + " -> "
							+ GraphUtils.getName(neighbor) + " is not equal. Source: " + edge.getCapacity()
							+ " compareTo: " + compareToEdge.getCapacity());
				}
			}
		}
		return true;
	...
	public static boolean normalEquals(FlowGraph source, FlowGraph compareTo) {
		for (Vertex vertex : source.getVertices()) {
			...
			for (Vertex neighbor : neighbors) {
				var edge = vertex.getEdge(neighbor);
				var compareToEdge = GraphUtils.getVertex(compareTo, GraphUtils.getName(vertex))
						.getEdge(GraphUtils.getVertex(compareTo, GraphUtils.getName(neighbor)));
				if (edge.getFlow() != compareToEdge.getFlow()) {
					return false;
				}
				if (edge.getCapacity() != compareToEdge.getCapacity()) {
