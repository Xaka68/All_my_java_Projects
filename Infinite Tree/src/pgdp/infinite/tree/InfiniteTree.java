...
public class InfiniteTree<T> {
	...
	public T find(T from, int maxDepth, Optimizable<T> optimizable) {
		...
		if (optimizable.process(from) == true) {

			return from;
		}
		InfiniteNode<T> root = withRoot(from);
		T a;
		while (root != null && root.isFullyCalculated() == false) {
			a = root.calculateNextChild().getValue();
			if (root.isFullyCalculated() == true) {
				root = null;
				System.gc();
			}
	...
	public static void main(String[] args) {
		...
		InfiniteNode<Long> root = binaryValueTree.withRoot(0L);
		root.calculateAllChildren();
		System.out.println(root.isFullyCalculated());

		System.out.println(root.calculateNextChild());
		System.out.println(root.calculateNextChild());
