...
public class InfiniteTreeTest {
	...
	public void findTest(int from, int depth, int searchVal, int expected, String message, String name) {
		...
		var optimizable = new OptimizableComparable<>(searchVal) {
			...
			public boolean process(Integer t) {
				// System.out.printf("%s, %s, %s\n", expectedBest.compareTo(t), expectedBest,
				// t);
	...
	private static Stream<Arguments> findTest() {
		return Stream.of(
				arguments(0, 420, 420, 420, "Your implementation does not search deep enough!", "searches deep enough"),
				arguments(0, 419, 420, 419, "Your implementation searches to deep!", "does not exceed search depth"),
	...
	/**
	 * This Test creates an Iterator over a List of String Arrays with a starting
	 * length of 100k and growing This Test should run without Problems for 2048MB
	 * RAM (add maxHeapSize = "2048m" to "test {}" in your build.gradle) if you
	 * don't delete your nodes it will throw an "OutOfMemoryError" and it'll be
	 * shown as an "ignored Test"
	 */
	...
	/**
	 * This Test creates an Iterator over a List of 100k Integers (every node has
	 * 100k children) You should delete every child after checking it This Test
	 * should run without Problems for 2048MB RAM (add maxHeapSize = "2048m" to your
	 * build.gradle) if you don't delete your nodes it will throw an
	 * "OutOfMemoryError" and it'll be shown as an "ignored Test"
	 */
	public void manyChildrenTreeTest() {
		...
		// Optimizable that finds the number closes to 2 (unimportant since we wanna
		// test if the test runs)
		var anyOptimizable = new pgdp.infinite.tree.OptimizableComparable<>(2);
