...
public class UnitTests {
	...
	public static Stream<Arguments> outOfBoundsTest() {
		...
		return Stream.of(arguments(outOfBounds1, emptyMove, false), arguments(outOfBounds2, emptyMove, false),
				arguments(outOfBounds3, emptyMove, false), arguments(outOfBounds4, emptyMove, false),
				arguments(outOfBounds5, emptyMove, false), arguments(outOfBounds6, emptyMove, false),
				arguments(move, outOfBounds1, true), arguments(move, outOfBounds2, true),
				arguments(move, outOfBounds3, true), arguments(move, outOfBounds4, true),
				arguments(move, outOfBounds5, true), arguments(move, outOfBounds6, true));
	}
	...
	public static Stream<Arguments> invalidMoveTest() {
		...
		return Stream.of(arguments(move1, invalid1, true), arguments(move1, invalid2, true),
				arguments(invalid3, move1, false), arguments(invalid4, move1, false));
	}
	...
	/**
	 * Creates a Move[] array from string. A move consists of three integers
	 * separates by commas. The first is the value, the second is the x coordinate,
	 * the third is the y coordinate.
	 * 
	 * @param s
	 * @return
	 */
	public static Move[] parseString(String s) {
		...
		for (int i = 0; i < split.length; i += 3) {
			moves[i / 3] = new Move(Integer.valueOf(split[i + 1]), Integer.valueOf(split[i + 2]),
