...
public class DataAccess {
	...
	public static Stream<TrainConnection> getDepartureBoardFor(Station station, LocalDateTime date) {
		BufferedReader in;
		try {
			String urlBuild = URL_DEPARTURE_BOARD + station.id()
					+ (date != null ? "&date=" + date.format(FORMATTER) : "");
			...
			Files.writeString(Path.of(
					"connections_test/" + (date != null ? date.format(FORMATTER) : "con" + Math.random()) + ".json"),
					content);
