...
public class CleanDatasetTest {
	...
	void shouldRemoveDuplicates() {
		final var result = DataProcessing
				.cleanDataset(Stream.of(JSONParser.getRe8BasicParsed(), JSONParser.getRe8BasicParsed())).toList();
	...
	void shouldKeepNonDuplicates() {
		final var result = DataProcessing
				.cleanDataset(Stream.of(JSONParser.getIceBasicParsed(), JSONParser.getRe8BasicParsed())).toList();
	...
	void shouldSortByDeparture() {
		final var result = DataProcessing
				.cleanDataset(Stream.of(JSONParser.getRe8BasicParsed(), JSONParser.getIceBasicParsed())).toList();
	...
	void shouldRemoveAllStopsThatWereCancelled() {
		final var result = DataProcessing.cleanDataset(Stream.of(JSONParser.getStr29CanceledParsed()))
	...
	void shouldNotRemoveStopsThatAreNotCancelled() {
		...
		final var result = DataProcessing.cleanDataset(Stream.of(ice881)).map(TrainConnection::stops);
